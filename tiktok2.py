class TikToks(TikTokApi):
    ...
    def get_trending_tiktoks(self, keywords_list):
        # returns tiktok dictionary/JSON
        # self.api = TikTokApi()
        # self.TikTokApi.get_instance(custom_verifyFp=fp, use_test_endpoints=True, use_selenium=True)
        for word in keywords_list:
            try:
                # convert string to bytes for search
                keyword = bytes(word, 'utf-8')
                trend = self.api.by_hashtag(keyword)
            except TypeError as err:
                logging.error(err, 'Keywords must be in bytes format')
...
tiktoks_df.set_index('postID')
api = TikTokApi.get_instance()
# tiktoks.get_tiktok_trends(keywords)
# key1 = keywords[1]
# trending = api.by_hashtag(key1)
# for tiktok in trending:
#     print(tiktok)

news.all_news_df['encoded_keys'] = news.all_news_df['keywords'].map(lambda x: x.encode('base64', 'strict'))

tiktoks["tags"] = news.all_news_df['encoded_keys'].apply(api.by_hashtag)
